...
export const getAllStudents = async (req, res) => {
  try {
    ...
    studentsData.sort((a, b) => 
      (b.aptitude_solved + b.dsa_solved) - (a.aptitude_solved + a.dsa_solved)
    );

    const totalStudents = studentsData.length;
    const totalPages = Math.ceil(totalStudents / limit);
    const paginatedData = studentsData.slice(skip, skip + limit);
...
export const getStudentsByBranch = async (req, res) => {
  try {
    ...
    const college = await College.findOne();
    const branchData = college.branches.get(branch);
...
export const getStudentsByYear = async (req, res) => {
  try {
    ...
    colleges.forEach((college) => {
      college.branches.forEach((branchData, branch) => {
        const yearData = branchData.years.get(year);
        if (yearData) {
          yearData.students.forEach((student) => {
            studentsData.push({
              name: student.username,
